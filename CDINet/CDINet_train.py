...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        ...
        if epoch > 80 and (epoch % 5 == 0 or epoch == opt.epoch):
            torch.save(model.module.state_dict(), save_path + 'CDINet_epoch_{}.pth'.format(epoch))
            torch.save(model.state_dict(), save_path + 'CDINet_epoch_{}.pth'.format(epoch))

    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        torch.save(model.module.state_dict(), save_path + 'CDINet_epoch_{}_checkpoint.pth'.format(epoch + 1))
